...
class TwoLayerNet(object):
    def __init__(self, input_size, hidden_size, output_size,
        ...
        self.params['W2'] = std * \
            torch.randn(hidden_size, output_size, dtype=dtype, device=device)
...
def nn_forward_pass(params, X):
    ...
    hidden = torch.max(torch.FloatTensor([0]).to(
        dtype).cuda(), (X.mm(W1)+b1.reshape(1, -1)).to(dtype))
...
def nn_train(params, loss_func, pred_func, X, y, X_val, y_val,
    ...
    for iter in range(num_iters):
        ...
        if verbose and iter % 100 == 0:
            print(f'iteration {iter} / {num_iters}: loss {loss.item()}')
