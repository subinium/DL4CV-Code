    ...
    def backward(dout, cache):
        ...
        for i in range(N):
            for f in range(F):
                ...
                for hh in range(0, H, stride):
                    for ww in range(0, W, stride):
                        dw[f] += x_pad[i, :, hh: hh + HH, ww: ww + WW] * \
                            dout[i, f, hh//stride, ww//stride]
                        dx_pad[i, :, hh:hh + HH, ww:ww + WW] += w[f] * \
                            dout[i, f, hh//stride, ww//stride]
            ...
            for c in range(C):
                for hh in range(0, H, stride):
                    for ww in range(0, W, stride):
                        ...
                        dx[i, c, hh:hh+pool_height, ww:ww +
                            pool_width][ind1, ind2] = dout[i, c, hh//stride, ww//stride]
                        #############################################################################
                        #                              END OF YOUR CODE                             #
                        #############################################################################
        return dx
    ...
    def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params['W1'] = weight_scale * \
            torch.randn(num_filters, C, filter_size,
                        filter_size).to(dtype).to(device)
        ...
        self.params['W2'] = weight_scale * \
            torch.randn(num_filters * H * W // 4,
                        hidden_dim).to(dtype).to(device)
        self.params['b2'] = torch.zeros(hidden_dim).to(dtype).to(device)
        self.params['W3'] = weight_scale * \
            torch.randn(hidden_dim, num_classes).to(dtype).to(device)
    ...
    def loss(self, X, y=None):
        ...
        loss += reg * \
            torch.sum(torch.tensor(
                [torch.sum(self.params['W%d' % i] ** 2) for i in [1, 2, 3]]))
...
class DeepConvNet(object):
            ...
            if i == 0:
                if weight_scale == "kaiming":
                    ...
                    self.params[f'W{i+1}'] = weight_scale * \
                        torch.randn(layers_dims[i], C, 3, 3).to(
                            dtype).to(device)
    ...
    def loss(self, X, y=None):
        ...
        while i >= 0:
            ...
            if self.batchnorm:
                dout, grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = SpatialBatchNorm.backward(
                    dout, cache_history.pop())
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        if relu:
            weight = (2/Din)**(1/2) * torch.randn(Din,
                                                  Dout, dtype=dtype, device=device)
        else:
            weight = (1/Din)**(1/2) * torch.randn(Din,
                                                  Dout, dtype=dtype, device=device)
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
        ...
        # Replace "pass" statement with your code
        if relu:
            weight = (2/(Din*K*K))**(1/2) * torch.randn(Din,
                                                        Dout, K, K, dtype=dtype, device=device)
        else:
            weight = (1/(Din*K*K))**(1/2) * torch.randn(Din,
                                                        Dout, K, K, dtype=dtype, device=device)
        ###########################################################################
        #                            END OF YOUR CODE                             #
        ###########################################################################
